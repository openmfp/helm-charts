...
kubectl create secret docker-registry ghcr-credentials -n openmfp-system --docker-server=ghcr.io --docker-username=$ghUser --docker-password=${GH_TOKEN} -o yaml > secret.yaml << EOF
# Dry-run check (optional)
# uncomment the following line if you want to see the generated YAML manifest without creating the secret
# --dry-run=client
EOF
